    ...
    */
    ...
    socket.on('join_room', (payload) => {
        ...
        if ((typeof username == 'undefined') || (username === null)) {
            response = {};
            response.result = 'fail';
            response.message = 'client did not send a valid username to join the chatroom';
            socket.emit('join_room_response', response);
            serverLog('join_room command failed', JSON.stringify(response));
            return;
        }
    ...
    socket.on('uninvite', (payload) => {
        ...
        if ((typeof requested_user == 'undefined') || (requested_user === null) || (requested_user === "")) {
            response = {
                result: 'fail',
                message: 'client did not send a valid user to invite to uninvite'
            }
            socket.emit('uninvited', response);
            serverLog('uninvite command failed', JSON.stringify(response));
            return;
    ...
    socket.on('game_start', (payload) => {
        ...
        if ((typeof requested_user == 'undefined') || (requested_user === null) || (requested_user === "")) {
            response = {
                result: 'fail',
                message: 'client did not send a valid user to invite to engage in play'
            }
            socket.emit('game_start_response', response);
            serverLog('game_start command failed', JSON.stringify(response));
            return;
...
/* Return true if r + dr supports playing at r and c + dcupports playing at c */
function adjacent_support(who, dr, dc, r, c, board) {
    ...
    /* Check to make sure that the adjacent suport is on the board */
    if ((r + dr < 0) || (r + dr > 7)) {
        return false;
    }
    if ((c + dc < 0) || (c + dc > 7)) {
    ...
    /* Check to make sure that the there is space for a matching color to capture tokens */
